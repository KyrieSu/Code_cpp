...
public:
	Mat_1x1(const Vec1<T>& x){ arr[0] = x; }
...
public:
	...
	Mat_1x1<T> operator * (const Mat4x1<T>& R)const	{ return Mul<Mat_1x4, Mat_4x1<T>, Mat_1x1<T>>(*this, R); }
	Mat_1x3<T> operator * (const Mat4x3<T>& R)const	{ return Mul<Mat_1x4, Mat_4x3<T>, Mat_1x3<T>>(*this, R); }
	Mat_1x4<T> operator * (const Mat4x4<T>& R)const	{ return Mul<Mat_1x4, Mat_4x4<T>, Mat_1x4>(*this, R); }
...
public:
	Mat_4x1(const Vec4<T>& x = Vec4<T>(0, 0, 0, 0))        { arr[0] = x; }
...
public:
	...
	Mat_3x3 Transpose() const								{ return _Transpose<Mat_x3, Mat_3x3>(*this); }
